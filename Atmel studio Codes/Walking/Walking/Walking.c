...
angle_upper_byte_servo12, angle_lower_byte_servo12,angle_upper_byte_servo13, angle_lower_byte_servo13,
angle_upper_byte_servo14, angle_lower_byte_servo14,angle_upper_byte_servo15, angle_lower_byte_servo15,
...
void walk1(void)
{
	...
	for(i=1; i<=45; i++)
	{
		...
		if(i<=10)
		{
			angle_servo3(S3-10+i);
		}
		_delay_ms(20);
	}
	
	//
	for(i=1; i<=21 ; i++) // 12. inversion of 1. swing left back to initial position
	{
		if(i<=7)
		{
		angle_servo1(S1+7-i);
	...
	for (i=1; i<=36; i++) // 8. inversion step 3 bend leg to initial
	{
		...
		if(i<=19)
		{
			angle_servo7(S7-17-i);
		}
		if (i==15)
		{
			
			angle_servo1(S1-l-9); //4. tilt left leg outward to prevent clash with right leg using servo 2
			angle_servo2(S2-l+13); // 5. tilt body right
		}
	...
	for(i=1; i<=20 ; i++) // 12. inversion of 1. swing left back to initial position
	{
		if(i<=2)
		{
			angle_servo2(S2-2+i);
	...
	for (i=1; i<=36; i++) // 8. inversion step 3 bend leg to initial
	{
		...
		if (i==15)
		{
			...
			angle_servo1(S1+r-10); //4. tilt left leg outward to prevent clash with right leg using servo 2
			angle_servo2(S2+r+4); // 5. tilt body right
		}
